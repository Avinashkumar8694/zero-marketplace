...
var Ge = (i, e, t) => Dt(i, typeof e != "symbol" ? e + "" : e, t);
...
(function(i) {
  (function(e) {
    ...
    }, w = Object.getPrototypeOf(Function), T = typeof Map == "function" && typeof Map.prototype.entries == "function" ? Map : Ct(), R = typeof Set == "function" && typeof Set.prototype.entries == "function" ? Set : Tt(), D = typeof WeakMap == "function" ? WeakMap : Rt(), W = s ? Symbol.for("@reflect-metadata:registry") : void 0, se = Mt(), Se = Ot(se);
    function ut(n, o, a, c) {
      if (m(a)) {
        if (!Ne(n))
          throw new TypeError();
    ...
    function Ot(n) {
      ...
      function c(d, f, p) {
        ...
        if (m(S)) {
          if (!p)
            return;
          if (S = new T(), $.set(f, S), !n.setProvider(d, f, a))
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const J = globalThis, Xe = (i) => i, le = J.trustedTypes, Ye = le ? le.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, it = "$lit$", U = `lit$${Math.random().toFixed(9).slice(2)}$`, st = "?" + U, Qt = `<${st}>`, z = document, te = () => z.createComment(""), re = (i) => i === null || typeof i != "object" && typeof i != "function", Ae = Array.isArray, Kt = (i) => Ae(i) || typeof (i == null ? void 0 : i[Symbol.iterator]) == "function", ve = `[ 	
\f\r]`, X = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, Je = /-->/g, Qe = />/g, j = RegExp(`>|${ve}(?:([^\\s"'>=/]+)(${ve}*=${ve}*(?:[^ 	
...
const rr = (i, e) => {
  ...
  for (let _ = 0; _ < t; _++) {
    const y = i[_];
    let A, b, w = -1, T = 0;
    for (; T < y.length && (l.lastIndex = T, b = l.exec(y), b !== null); ) T = l.lastIndex, l === X ? b[1] === "!--" ? l = Je : b[1] !== void 0 ? l = Qe : b[2] !== void 0 ? (ot.test(b[2]) && (s = RegExp("</" + b[2], "g")), l = j) : b[3] !== void 0 && (l = j) : l === j ? b[0] === ">" ? (l = s ?? X, w = -1) : b[1] === void 0 ? w = -2 : (w = l.lastIndex - b[2].length, A = b[1], l = b[3] === void 0 ? j : b[3] === '"' ? et : Ke) : l === et || l === Ke ? l = j : l === Je || l === Qe ? l = X : (l = j, s = void 0);
    const R = l === j && i[_ + 1].startsWith("/>") ? " " : "";
    u += l === X ? y + Qt : w >= 0 ? (r.push(A), y.slice(0, w) + it + y.slice(w) + U + R) : y + U + (w === -2 ? _ : R);
  }
...
class ne {
  constructor({ strings: e, _$litType$: t }, r) {
    ...
    if (this.el = ne.createElement(A, r), H.currentNode = this.el.content, t === 2 || t === 3) {
    ...
    for (; (s = H.nextNode()) !== null && y.length < _; ) {
      if (s.nodeType === 1) {
        if (s.hasAttributes()) for (const w of s.getAttributeNames()) if (w.endsWith(it)) {
          const T = b[l++], R = s.getAttribute(w).split(U), D = /([.?@])?(.*)/.exec(T);
          y.push({ type: 1, index: u, name: D[2], strings: R, ctor: D[1] === "." ? ir : D[1] === "?" ? sr : D[1] === "@" ? or : ce }), s.removeAttribute(w);
...
class nr {
  ...
  u(e) {
    ...
    for (; y !== void 0; ) {
      if (l === y.index) {
        let A;
        y.type === 2 ? A = new ie(u, u.nextSibling, this, e) : y.type === 1 ? A = new y.ctor(u, y.name, y.strings, this, e) : y.type === 6 && (A = new ar(u, this, e)), this._$AV.push(A), y = r[++_];
      }
...
class ce {
  ...
  _$AI(e, t = this, r, s) {
    ...
    if (u === void 0) e = q(this, e, t, 0), l = !re(e) || e !== this._$AH && e !== F, l && (this._$AH = e);
      ...
      const _ = e;
      let y, A;
      for (e = u[0], y = 0; y < u.length - 1; y++) A = q(this, _[r + y], t, y), A === F && (A = this._$AH[y]), l || (l = !re(A) || A !== this._$AH[y]), A === C ? e = C : e !== C && (e += (A ?? "") + u[y + 1]), this._$AH[y] = A;
    }
    l && !s && this.j(e);
  }
  j(e) {
...
var hr = Object.defineProperty, dr = Object.getOwnPropertyDescriptor, G = (i, e, t, r) => {
...
let N = class extends Q {
  ...
  render() {
    return tr`<div class="node"><div class="eyebrow">DB</div><strong>${this.operation}</strong><div>${this.source}${this.model ? ` • ${this.model}` : ""}</div></div>`;
  }
};
N.styles = Wt`:host{display:block}.node{padding:14px 16px;border-radius:16px;border:1px solid rgba(14,116,144,.2);background:#f0fdfa;color:#0f172a}.eyebrow{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#0f766e;font-weight:700}`;
G([
  Ee({ type: String })
...
G([
  ge({ attributeType: ee.PROPERTY, uiComponentType: K.DROPDOWN, displayLabel: "Operation", fieldMappings: "operation", optionItems: [{ label: "Query", value: "query" }, { label: "Insert", value: "insert" }, { label: "Update", value: "update" }, { label: "Delete", value: "delete" }] })
], N.prototype, "operationConfig", 1);
G([
  ge({ attributeType: ee.PROPERTY, uiComponentType: K.TEXT_INPUT, displayLabel: "Data Source", fieldMappings: "source" })
], N.prototype, "sourceConfig", 1);
G([
  ge({ attributeType: ee.PROPERTY, uiComponentType: K.TEXT_INPUT, displayLabel: "Model", fieldMappings: "model" })
], N.prototype, "modelConfig", 1);
N = G([
  jt({ name: "zero-flow-node-db", version: "1.0.0", title: "DB Node", elementSelector: "zero-flow-node-db", group: "Flow Nodes", iconName: "flow-db-node.png" }),
  Ht()
], N);
export {
  N as ZeroFlowNodeDb
