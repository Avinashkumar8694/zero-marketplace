...
(function(i) {
  (function(e) {
    ...
    function St() {
      var n;
      !m(G) && typeof t.Reflect < "u" && !(G in t.Reflect) && typeof t.Reflect.defineMetadata == "function" && (n = Pt(t.Reflect));
      var o, a, c, v = new N(), P = {
        registerProvider: O,
        getProvider: h,
        setProvider: f
    ...
    function Pt(n) {
      var o = n.defineMetadata, a = n.hasOwnMetadata, c = n.getOwnMetadata, v = n.getOwnMetadataKeys, P = n.deleteMetadata, O = new N(), g = {
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Vt, defineProperty: Ft, getOwnPropertyDescriptor: qt, getOwnPropertyNames: Zt, getOwnPropertySymbols: Xt, getPrototypeOf: Yt } = Object, I = globalThis, Ve = I.trustedTypes, Jt = Ve ? Ve.emptyScript : "", ye = I.reactiveElementPolyfillSupport, X = (i, e) => i, ue = { toAttribute(i, e) {
  switch (e) {
    case Boolean:
      i = i ? Jt : null;
      break;
...
\f\r"'\`<>=]|("|')|))|$)`, "g"), Je = /'/g, Qe = /"/g, st = /^(?:script|style|textarea|title)$/i, er = (i) => (e, ...t) => ({ _$litType$: i, strings: e, values: t }), tr = er(1), V = Symbol.for("lit-noChange"), C = Symbol.for("lit-nothing"), Ke = /* @__PURE__ */ new WeakMap(), D = L.createTreeWalker(L, 129);
function ot(i, e) {
...
const rr = (i, e) => {
  ...
  for (let _ = 0; _ < t; _++) {
    ...
    u += l === Z ? y + Qt : w >= 0 ? (r.push(A), y.slice(0, w) + nt + y.slice(w) + U + R) : y + U + (w === -2 ? _ : R);
  }
    ...
    for (; (s = D.nextNode()) !== null && y.length < _; ) {
      if (s.nodeType === 1) {
        if (s.hasAttributes()) for (const w of s.getAttributeNames()) if (w.endsWith(nt)) {
          const T = b[l++], R = s.getAttribute(w).split(U), N = /([.?@])?(.*)/.exec(T);
...
let z = class extends J {
  ...
  render() {
    return tr`<div class="node"><div class="eyebrow">Emit</div><strong>${this.eventName}</strong><div>${this.payload}</div></div>`;
  }
};
z.styles = Wt`:host{display:block}.node{padding:14px 16px;border-radius:16px;border:1px solid rgba(16,185,129,.2);background:#ecfdf5;color:#0f172a}.eyebrow{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#059669;font-weight:700}`;
re([
  at({ type: String })
], z.prototype, "eventName", 2);
re([
  at({ type: String })
], z.prototype, "payload", 2);
re([
  tt({ attributeType: ae.PROPERTY, uiComponentType: oe.TEXT_INPUT, displayLabel: "Event Name", fieldMappings: "eventName" })
], z.prototype, "eventNameConfig", 1);
re([
  tt({ attributeType: ae.PROPERTY, uiComponentType: oe.TEXTAREA, displayLabel: "Payload", fieldMappings: "payload" })
], z.prototype, "payloadConfig", 1);
z = re([
  Dt({ name: "zero-flow-node-emit", version: "1.0.0", title: "Emit Node", elementSelector: "zero-flow-node-emit", group: "Flow Nodes", iconName: "flow-emit-node.png" }),
  Ht()
], z);
export {
  z as ZeroFlowNodeEmit
};
