 ...
 */
const ie = globalThis, me = ie.ShadowRoot && (ie.ShadyCSS === void 0 || ie.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, ge = Symbol(), Ge = /* @__PURE__ */ new WeakMap();
let et = class {
  constructor(e, t, n) {
...
let L = class extends Y {
  constructor() {
    super(...arguments), this.eventName = "flow.completed", this.payload = "result";
  }
  get eventNameConfig() {
    return this.eventName;
  }
  set eventNameConfig(i) {
    this.eventName = i || "flow.completed";
  }
  get payloadConfig() {
    return this.payload;
  }
  set payloadConfig(i) {
    this.payload = i || "result";
  }
  render() {
    return Kt`<div class="node"><div class="eyebrow">Emit</div><strong>${this.eventName}</strong><div>${this.payload}</div></div>`;
  }
};
L.styles = zt`:host{display:block}.node{padding:14px 16px;border-radius:16px;border:1px solid rgba(16,185,129,.2);background:#ecfdf5;color:#0f172a}.eyebrow{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#059669;font-weight:700}`;
te([
  st({ type: String })
], L.prototype, "eventName", 2);
te([
  st({ type: String })
], L.prototype, "payload", 2);
te([
  Ke({ attributeType: oe.PROPERTY, uiComponentType: se.TEXT_INPUT, displayLabel: "Event Name", fieldMappings: "eventName" })
], L.prototype, "eventNameConfig", 1);
te([
  Ke({ attributeType: oe.PROPERTY, uiComponentType: se.TEXTAREA, displayLabel: "Payload", fieldMappings: "payload" })
], L.prototype, "payloadConfig", 1);
L = te([
  It({ name: "zero-flow-node-emit", version: "1.0.0", title: "Emit Node", elementSelector: "zero-flow-node-emit", group: "Flow Nodes", iconName: "flow-emit-node.png" }),
  jt()
], L);
export {
  L as ZeroFlowNodeEmit
};
