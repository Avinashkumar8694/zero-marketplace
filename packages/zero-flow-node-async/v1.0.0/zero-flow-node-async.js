...
(function(i) {
  (function(e) {
    ...
    }, w = Object.getPrototypeOf(Function), R = typeof Map == "function" && typeof Map.prototype.entries == "function" ? Map : Ct(), T = typeof Set == "function" && typeof Set.prototype.entries == "function" ? Set : Rt(), N = typeof WeakMap == "function" ? WeakMap : Tt(), G = s ? Symbol.for("@reflect-metadata:registry") : void 0, ne = St(), Ae = Ot(ne);
    function ut(n, o, a, c) {
      if (g(a)) {
        if (!ke(n))
          throw new TypeError();
    ...
    function Mt() {
      ...
      function h(p, $) {
        var E = v.get(p), M;
        return g(E) || (M = E.get($)), g(M) && (M = m(p, $), g(M) || (g(E) && (E = new R(), v.set(p, E)), E.set($, M))), M;
      }
      function d(p) {
        if (g(p))
          throw new TypeError();
    ...
    function St() {
      var n;
      return !g(G) && S(t.Reflect) && Object.isExtensible(t.Reflect) && (n = t.Reflect[G]), g(n) && (n = Mt()), !g(G) && S(t.Reflect) && Object.isExtensible(t.Reflect) && Object.defineProperty(t.Reflect, G, {
        enumerable: !1,
        configurable: !1,
        writable: !1,
        value: n
    ...
    function Ot(n) {
      ...
      function c(d, f, p) {
        ...
        if (g(M)) {
          if (!p)
            return;
          if (M = new R(), $.set(f, M), !n.setProvider(d, f, a))
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Vt, defineProperty: Ft, getOwnPropertyDescriptor: qt, getOwnPropertyNames: Zt, getOwnPropertySymbols: Yt, getPrototypeOf: Xt } = Object, U = globalThis, Ve = U.trustedTypes, Jt = Ve ? Ve.emptyScript : "", ye = U.reactiveElementPolyfillSupport, Y = (i, e) => i, ue = { toAttribute(i, e) {
  switch (e) {
    case Boolean:
      i = i ? Jt : null;
      break;
...
let B = class extends HTMLElement {
  ...
  static finalize() {
    ...
    if (this.finalized = !0, this._$Ei(), this.hasOwnProperty(Y("properties"))) {
      const t = this.properties, r = [...Zt(t), ...Yt(t)];
...
const rr = (i, e) => {
  ...
  for (let _ = 0; _ < t; _++) {
    const y = i[_];
    let A, b, w = -1, R = 0;
    for (; R < y.length && (l.lastIndex = R, b = l.exec(y), b !== null); ) R = l.lastIndex, l === Z ? b[1] === "!--" ? l = Ye : b[1] !== void 0 ? l = Xe : b[2] !== void 0 ? (st.test(b[2]) && (s = RegExp("</" + b[2], "g")), l = I) : b[3] !== void 0 && (l = I) : l === I ? b[0] === ">" ? (l = s ?? Z, w = -1) : b[1] === void 0 ? w = -2 : (w = l.lastIndex - b[2].length, A = b[1], l = b[3] === void 0 ? I : b[3] === '"' ? Qe : Je) : l === Qe || l === Je ? l = I : l === Ye || l === Xe ? l = Z : (l = I, s = void 0);
    const T = l === I && i[_ + 1].startsWith("/>") ? " " : "";
    u += l === Z ? y + Qt : w >= 0 ? (r.push(A), y.slice(0, w) + nt + y.slice(w) + D + T) : y + D + (w === -2 ? _ : T);
  }
    ...
    for (; (s = j.nextNode()) !== null && y.length < _; ) {
      if (s.nodeType === 1) {
        if (s.hasAttributes()) for (const w of s.getAttributeNames()) if (w.endsWith(nt)) {
          const R = b[l++], T = s.getAttribute(w).split(D), N = /([.?@])?(.*)/.exec(R);
          y.push({ type: 1, index: u, name: N[2], strings: T, ctor: N[1] === "." ? ir : N[1] === "?" ? sr : N[1] === "@" ? or : ce }), s.removeAttribute(w);
        } else w.startsWith(D) && (y.push({ type: 6, index: u }), s.removeAttribute(w));
        if (st.test(s.tagName)) {
          const w = s.textContent.split(D), R = w.length - 1;
          if (R > 0) {
        ...
        let w = -1;
        for (; (w = s.data.indexOf(D, w + 1)) !== -1; ) y.push({ type: 7, index: u }), w += D.length - 1;
      }
      u++;
    }
  }
...
let z = class extends J {
  ...
  render() {
    return tr`<div class="node"><div class="eyebrow">Async</div><strong>${this.strategy}</strong><div>${this.delayMs} ms</div></div>`;
  }
};
z.styles = Wt`:host{display:block}.node{padding:14px 16px;border-radius:16px;border:1px solid rgba(8,145,178,.2);background:#ecfeff;color:#0f172a}.eyebrow{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#0e7490;font-weight:700}`;
re([
  at({ type: String })
], z.prototype, "strategy", 2);
re([
  at({ type: Number })
], z.prototype, "delayMs", 2);
re([
  tt({ attributeType: ae.PROPERTY, uiComponentType: oe.DROPDOWN, displayLabel: "Strategy", fieldMappings: "strategy", optionItems: [{ label: "Queue", value: "queue" }, { label: "Debounce", value: "debounce" }, { label: "Retry", value: "retry" }, { label: "Schedule", value: "schedule" }] })
], z.prototype, "strategyConfig", 1);
re([
  tt({ attributeType: ae.PROPERTY, uiComponentType: oe.NUMBER_INPUT, displayLabel: "Delay (ms)", fieldMappings: "delayMs" })
], z.prototype, "delayMsConfig", 1);
z = re([
  jt({ name: "zero-flow-node-async", version: "1.0.0", title: "Async Node", elementSelector: "zero-flow-node-async", group: "Flow Nodes", iconName: "flow-async-node.png" }),
  Ht()
], z);
export {
  z as ZeroFlowNodeAsync
};
