...
(function(i) {
  (function(e) {
    ...
    }, $ = Object.getPrototypeOf(Function), T = typeof Map == "function" && typeof Map.prototype.entries == "function" ? Map : Mt(), R = typeof Set == "function" && typeof Set.prototype.entries == "function" ? Set : Pt(), N = typeof WeakMap == "function" ? WeakMap : Ct(), z = o ? Symbol.for("@reflect-metadata:registry") : void 0, re = Et(), be = St(re);
    function ot(r, s, a, c) {
      if (_(a)) {
        if (!Re(r))
          throw new TypeError();
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: Wt, defineProperty: Bt, getOwnPropertyDescriptor: Ft, getOwnPropertyNames: Vt, getOwnPropertySymbols: qt, getPrototypeOf: Zt } = Object, j = globalThis, Be = j.trustedTypes, Xt = Be ? Be.emptyScript : "", pe = j.reactiveElementPolyfillSupport, Z = (i, e) => i, ae = { toAttribute(i, e) {
  switch (e) {
    case Boolean:
      i = i ? Xt : null;
      break;
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const X = globalThis, ue = X.trustedTypes, Ve = ue ? ue.createPolicy("lit-html", { createHTML: (i) => i }) : void 0, tt = "$lit$", U = `lit$${Math.random().toFixed(9).slice(2)}$`, rt = "?" + U, Yt = `<${rt}>`, D = document, J = () => D.createComment(""), Q = (i) => i === null || typeof i != "object" && typeof i != "function", $e = Array.isArray, Jt = (i) => $e(i) || typeof (i == null ? void 0 : i[Symbol.iterator]) == "function", ye = `[ 	
\f\r]`, q = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, qe = /-->/g, Ze = />/g, I = RegExp(`>|${ye}(?:([^\\s"'>=/]+)(${ye}*=${ye}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), Xe = /'/g, Ye = /"/g, nt = /^(?:script|style|textarea|title)$/i, Qt = (i) => (e, ...t) => ({ _$litType$: i, strings: e, values: t }), Kt = Qt(1), B = Symbol.for("lit-noChange"), C = Symbol.for("lit-nothing"), Je = /* @__PURE__ */ new WeakMap(), H = D.createTreeWalker(D, 129);
function it(i, e) {
...
const er = (i, e) => {
  ...
  for (let w = 0; w < t; w++) {
    const v = i[w];
    let b, A, $ = -1, T = 0;
    for (; T < v.length && (u.lastIndex = T, A = u.exec(v), A !== null); ) T = u.lastIndex, u === q ? A[1] === "!--" ? u = qe : A[1] !== void 0 ? u = Ze : A[2] !== void 0 ? (nt.test(A[2]) && (o = RegExp("</" + A[2], "g")), u = I) : A[3] !== void 0 && (u = I) : u === I ? A[0] === ">" ? (u = o ?? q, $ = -1) : A[1] === void 0 ? $ = -2 : ($ = u.lastIndex - A[2].length, b = A[1], u = A[3] === void 0 ? I : A[3] === '"' ? Ye : Xe) : u === Ye || u === Xe ? u = I : u === qe || u === Ze ? u = q : (u = I, o = void 0);
    const R = u === I && i[w + 1].startsWith("/>") ? " " : "";
    l += u === q ? v + Yt : $ >= 0 ? (n.push(b), v.slice(0, $) + tt + v.slice($) + U + R) : v + U + ($ === -2 ? w : R);
  }
...
let L = class extends Y {
  ...
  render() {
    return Kt`<div class="node"><div class="eyebrow">Script</div><strong>${this.language}</strong><pre>${this.code}</pre></div>`;
  }
};
L.styles = zt`:host{display:block}.node{padding:14px 16px;border-radius:16px;border:1px solid rgba(15,23,42,.16);background:#fff;color:#0f172a}.eyebrow{font-size:.72rem;text-transform:uppercase;letter-spacing:.12em;color:#334155;font-weight:700}pre{margin:8px 0 0;background:#0f172a;color:#e2e8f0;border-radius:12px;padding:10px;white-space:pre-wrap}`;
te([
  st({ type: String })
...
te([
  Ke({ attributeType: oe.PROPERTY, uiComponentType: se.DROPDOWN, displayLabel: "Language", fieldMappings: "language", optionItems: [{ label: "TypeScript", value: "typescript" }, { label: "JavaScript", value: "javascript" }] })
], L.prototype, "languageConfig", 1);
te([
  Ke({ attributeType: oe.PROPERTY, uiComponentType: se.TEXTAREA, displayLabel: "Code", fieldMappings: "code" })
], L.prototype, "codeConfig", 1);
L = te([
  jt({ name: "zero-flow-node-script", version: "1.0.0", title: "Script Node", elementSelector: "zero-flow-node-script", group: "Flow Nodes", iconName: "flow-script-node.png" }),
  It()
], L);
export {
  L as ZeroFlowNodeScript
};
